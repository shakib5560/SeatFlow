...
export class AdminBookingsService {
  ...
  constructor(private readonly adminBookingsRepository: AdminBookingsRepository) {}
   ...
   */
  async listPendingBookings(query: AdminBookingQueryDto): Promise<PaginatedAdminBookingsDto> {
    const forcedQuery: AdminBookingQueryDto = { ...query, status: BookingStatus.PENDING };
   ...
   */
  async listAllBookings(query: AdminBookingQueryDto): Promise<PaginatedAdminBookingsDto> {
  ...
  async getBookingById(bookingId: string): Promise<AdminBookingDetailDto> {
    ...
    const booking = await this.adminBookingsRepository.findBookingById(bookingId);
    if (!booking) {
   ...
   */
  async approveBooking(bookingId: string, _dto: BookingApprovalDto): Promise<AdminActionResultDto> {
    this.logger.log(`Admin approval initiated: bookingId=${bookingId}`);

    // ── Guard 1: Existence check ──────────────────────────────────────────────
    const booking = await this.adminBookingsRepository.findBookingById(bookingId);
    if (!booking) {
      this.logger.warn(`Approval failed — booking not found: bookingId=${bookingId}`);
      throw new NotFoundException(`Booking with ID ${bookingId} does not exist.`);
    }
    ...
    if (result.status === BookingStatus.PENDING) {
      ...
      this.logger.error(`Approval produced no state change: bookingId=${bookingId}`);
      throw new ConflictException('Approval failed due to an unexpected state transition.');
    }
   ...
   */
  async rejectBooking(bookingId: string, _dto: BookingApprovalDto): Promise<AdminActionResultDto> {
    this.logger.log(`Admin rejection initiated: bookingId=${bookingId}`);

    // ── Guard 1: Existence check ──────────────────────────────────────────────
    const booking = await this.adminBookingsRepository.findBookingById(bookingId);
    if (!booking) {
      this.logger.warn(`Rejection failed — booking not found: bookingId=${bookingId}`);
      throw new NotFoundException(`Booking with ID ${bookingId} does not exist.`);
    }
  ...
  private async buildPaginatedResponse(query: AdminBookingQueryDto): Promise<PaginatedAdminBookingsDto> {
    ...
    const { data, totalItems } = await this.adminBookingsRepository.findBookings(query);

    const totalPages      = Math.ceil(totalItems / limit);
    const hasNextPage     = page < totalPages;
    const hasPreviousPage = page > 1;

    this.logger.log(`Admin booking list retrieved: totalItems=${totalItems}, totalPages=${totalPages}`);
